...
from .sg2sc_diffusion_edit import Sg2ScTransformerConditionWrapper
from .clip_encoders import CLIPTextEncoder, T5TextEncoder

from src.utils.logger import StatsLogger
from lightning.pytorch import LightningModule

class SgObjfeatVQDiffusionEdit(LightningModule):
    def __init__(self, config, num_objs, num_preds, pred_save_folder=None, text_encoder="t5", num_timesteps = 100) -> None:
        super().__init__()
        ...
        self.model = SgObjfeatVQDiffusion(
            num_objs=num_objs,
            num_preds=num_preds,
            num_timesteps = num_timesteps,
            text_emb_dim=self.clip_model.text_emb_dim,
            use_global_condition=text_encoder=="clip",
        )
    ...
    def get_condition(self, source_params, instructions: List[str]) -> Tensor:
        ...
        source_o = source_params["objfeat_vq_indices"]  # (B, N, K)
        ...
        B, N, K = source_o.shape
        o_emb = self.condition_objfeat_embed(source_o)  # (B, N, K, Do)
        ...
        for block in self.condition_objfeat_pool_transformer:
            o_emb = block(o_emb)
        o_emb = self.condition_objfeat_pool_norm(o_emb)  # (B*N, K, Do)
        o_emb = torch.mean(o_emb, dim=1).reshape(B, N, -1)  # (B, N, Do); average over objfeat tokens
        o_emb = self.condition_objfeat_pool(o_emb)  # (B, N, attn_dim)
        ...
        source_condition = self.condition_network(source_boxes, source_x, source_e, o_emb, mask=source_mask, condition=text_last_hidden_state)
    ...
    def forward(self, batch):
        ...
        target_params = batch["targets"]
        instructions = batch["instructions"]

        text_last_hidden_state, text_embeds = self.get_text_condition(instructions, self.device)
        loss_dict = self.model.compute_losses_concat(target_params, source_params, text_last_hidden_state, text_embeds)
        return loss_dict

    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.train_hypers['optimizer']['lr'])
        return optimizer
    ...
    def generate_samples(self, batch_size: int, num_nodes: int, source_params, instructions, cfg_scale=1., source_cfg_scale=1.):

        text_last_hidden_state, text_embeds = self.get_text_condition(instructions, self.device)
        generated_x, generated_e, generated_token_o = self.model.generate_samples(batch_size, num_nodes, source_params, text_last_hidden_state, text_embeds, cfg_scale=cfg_scale, source_cfg_scale=source_cfg_scale)
        
        return generated_x, generated_e, generated_token_o
    
    def post_process(self, objs, edges, objfeat_vq_indices):
        ...
        obj_masks = (objs != self.num_objs).long()  # (bs, n)
        # Mask and symmetrize edges
        edges = edges.argmax(dim=-1)  # (bs, n*(n-1)/2)
        edges = F.one_hot(edges, num_classes=self.num_preds+1).float()
        ...
        edges_negative = edges[..., 
            [*range(self.num_preds//2, self.num_preds)] + \
            [*range(0, self.num_preds//2)] + \
            [*range(self.num_preds, edges.shape[-1])]
        ]  # (bs, n, n, n_pred_types+1)
        edges = edges + edges_negative.permute(0, 2, 1, 3)
        edge_mask = torch.eye(objs.shape[-1], device=objs.device).bool().unsqueeze(0).unsqueeze(-1)  # (1, n, n, 1)
        ...
        return objs, obj_masks, edges, objfeat_vq_indices
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        for k in list(current_state_dict.keys()):
            if "clip_model" in k:
                checkpoint["state_dict"][k] = current_state_dict[k]
...
class SgObjfeatVQDiffusion(nn.Module):
    def __init__(self,
        ...
        self.network = SgObjfeatTransformerVQDiffusionWrapper(
            ...
            global_condition_dim=text_emb_dim if use_global_condition else None,
    ...
    def compute_losses(self,
        ...
        source_conditions: Optional[Tensor]=None,
        edge_weight=10.,
        objfeat_weight=10.
    ):
        # Unpack sample parameters
        x = sample_params["objs"]
        e = sample_params["flatten_edges"]
        o = sample_params["objfeat_vq_indices"]
        o = torch.cat([o[:, i] for i in range(o.shape[1])], dim=-1)  # (B, N*4)
        ...
        if self.training and self.cfg_drop_ratio > 0.:
            ...
            empty_source_conditions = torch.zeros_like(source_conditions[0]).unsqueeze(0)  # e.g., for CLIP ViT-B/32: (1, 77, 512)
            empty_prob = torch.rand(B, device=x.device) < self.cfg_drop_ratio
            source_conditions[empty_prob, ...] = empty_source_conditions
        ...
        if source_conditions is not None:
            cross_conditions = torch.cat([source_conditions, text_last_hidden_state], dim=1)
        else:
            cross_conditions = text_last_hidden_state

        _, _, _, vb_loss_x, vb_loss_e, vb_loss_o = self._train_loss(x, e, o, cross_conditions, text_embeds, edge_weight=edge_weight, objfeat_weight=objfeat_weight)
        ...
        losses["vb_o"] = objfeat_weight * vb_loss_o.mean() / o.shape[-1] 
    ...
    def compute_losses_concat(self,
        sample_params: Dict[str, Tensor],
        ...
        source_o = source_params["objfeat_vq_indices"]
        # Unpack sample parameters
        x = sample_params["objs"]
        e = sample_params["flatten_edges"]
        o = sample_params["objfeat_vq_indices"]
        ...
        if self.training and self.cfg_drop_ratio > 0.:
            ...
            empty_source_o = torch.ones_like(source_o[0]).unsqueeze(0)*64
            empty_source_e = torch.ones_like(source_e[0]).unsqueeze(0)*self.num_preds
            empty_prob = torch.rand(B, device=x.device) < self.cfg_drop_ratio
            source_o[empty_prob, ...] = empty_source_o
            source_e[empty_prob, ...] = empty_source_e

        _, _, _, vb_loss_x, vb_loss_e, vb_loss_o = self._train_loss(x, e, o, text_last_hidden_state, text_embeds, source_x, source_e, source_o, edge_weight, objfeat_weight)
        assert vb_loss_x.shape == vb_loss_e.shape == vb_loss_o.shape == (B,)
    ...
    def generate_samples(self,
        ...
        source_params: Dict[str, Tensor]=None,
        text_last_hidden_state: Optional[Tensor]=None,
        text_embeds: Optional[Tensor]=None,
        filter_ratio=0.,
        cfg_scale=1.,
        ...
        if source_params is not None:
            ...
            source_x = source_e = source_o = None
        ...
        if start_step == 0:
            ...
            if skip_step == 0:
                for diffusion_index in tqdm(range(start_step-1, -1, -1), desc="Generating graphs", ncols=125):
                    ...
                    log_z_x, log_z_e, log_z_o = self.p_sample(
                        ...
                        cfg_scale, source_cfg_scale, truncation_rate,
                        source_x, source_e, source_o
                    )
            # Fast sampling by skipping some steps
                ...
                for diffusion_index in tqdm(diffusion_list, desc="Generating graphs", ncols=125):
                    t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                    log_x_recon, log_e_recon, log_o_recon = self.predict_start_concat(
                        log_z_x, log_z_e, log_z_o, t,
                        source_x, source_e, source_o,
                        text_last_hidden_state, text_embeds,
                        cfg_scale, source_cfg_scale, truncation_rate
    ...
    def predict_start_concat(self,
        log_x_t: Tensor, log_e_t: Tensor, log_o_t: Tensor, t: LongTensor,
        source_x: Tensor, source_e: Tensor, source_o: Tensor,
        cond_emb: Tensor, global_cond_emb: Optional[Tensor]=None,
        cfg_scale=1., source_cfg_scale=2., truncation_rate=1.
    ):  # p(x0|xt)
        x_t: LongTensor = log_onehot_to_index(log_x_t)
        e_t: LongTensor = log_onehot_to_index(log_e_t)
        o_t: LongTensor = log_onehot_to_index(log_o_t)
        ...
        # Prepare for classifier-free guidance
        if cfg_scale != 1.:
            # empty_source_o = torch.zeros_like(source_o)

            empty_cond_emb = torch.zeros_like(cond_emb)  # e.g., for CLIP ViT-B/32: (B, 77, 512)
            empty_source_o = torch.ones_like(source_o)*64
            empty_source_e = torch.ones_like(source_e)*self.num_preds
            cond_emb = torch.cat([empty_cond_emb, empty_cond_emb, cond_emb], dim=0)
            x_t = torch.cat([x_t, x_t, x_t], dim=0)
            e_t = torch.cat([e_t, e_t, e_t], dim=0)
            o_t = torch.cat([o_t, o_t, o_t], dim=0)
            t = torch.cat([t, t, t], dim=0)
            source_x = torch.cat([source_x, source_x, source_x], dim=0)
            source_e = torch.cat([empty_source_e, source_e, source_e], dim=0)
            source_o = torch.cat([empty_source_o, source_o, source_o], dim=0)
            # source_o = torch.cat([source_o, source_o], dim=0)
            if global_cond_emb is not None:
                empty_global_cond_emb = torch.zeros_like(global_cond_emb)  # e.g., for CLIP ViT-B/32: (B, 512)
                global_cond_emb = torch.cat([empty_global_cond_emb, empty_global_cond_emb, global_cond_emb], dim=0)

        out_x, out_e, out_o = self.network(x_t, e_t, o_t, t, cond_emb, global_cond_emb, source_x, source_e, source_o)
        out_x: Tensor; out_e: Tensor; out_o: Tensor
        ...
        if cfg_scale != 1.:
            log_pred_x_uncond, log_pred_x_source_cond, log_pred_x_cond = log_pred_x.chunk(3, dim=0)
            log_pred_x = log_pred_x_uncond + source_cfg_scale * (log_pred_x_source_cond - log_pred_x_uncond) + cfg_scale * (log_pred_x_cond - log_pred_x_source_cond)
            log_pred_x -= torch.logsumexp(log_pred_x, dim=1, keepdim=True)  # normalize
            log_pred_e_uncond, log_pred_e_source_cond, log_pred_e_cond = log_pred_e.chunk(3, dim=0)
            log_pred_e = log_pred_e_uncond + source_cfg_scale * (log_pred_e_source_cond - log_pred_e_uncond) + cfg_scale * (log_pred_e_cond - log_pred_e_source_cond)
            log_pred_e -= torch.logsumexp(log_pred_e, dim=1, keepdim=True)  # normalize
            log_pred_o_uncond, log_pred_o_source_cond, log_pred_o_cond = log_pred_o.chunk(3, dim=0)
            log_pred_o = log_pred_o_uncond + source_cfg_scale * (log_pred_o_source_cond - log_pred_o_uncond) + cfg_scale * (log_pred_o_cond - log_pred_o_source_cond)
            log_pred_o -= torch.logsumexp(log_pred_o, dim=1, keepdim=True)  # normalize
            t, cond_emb = t[:batch_size, ...], cond_emb[:batch_size, ...]
            if global_cond_emb is not None:
                global_cond_emb = global_cond_emb[:batch_size, ...]
    ...
    def p_pred(self,
        ...
        cfg_scale=1., source_cfg_scale=2., truncation_rate=1.,
        source_x: Optional[Tensor]=None, source_e: Optional[Tensor]=None, source_o: Optional[Tensor]=None
    ...
    def _train_loss(self,
        ...
        cond_emb: Tensor, global_cond_emb: Optional[Tensor]=None,
        source_x: Tensor=None, source_e: Tensor=None, source_o: Tensor=None,
        edge_weight=1.,
        objfeat_weight=1.
    ):  # get the KL loss
        ...
        if source_x is not None:
            log_x0_recon, log_e0_recon, log_o0_recon = self.predict_start_concat(log_xt, log_et, log_ot, t, source_x, source_e, source_o, cond_emb, global_cond_emb)  # P_theta(x0|xt)
        else:
        ...
        Lt2 = (kl_loss_x + edge_weight * kl_loss_e + objfeat_weight * kl_loss_o).pow(2)  # (B,);   # TODO: add `objfeat_weight`
...
class SgObjfeatTransformerVQDiffusionWrapper(nn.Module):
    ...
    def forward(self,
        ...
        source_x: Optional[LongTensor]=None, source_e: Optional[LongTensor]=None, source_o: Optional[LongTensor]=None,
        ...
        if source_e is not None:
            new_e = e.repeat(1, 2, 2)
            source_e = source_e.repeat(1, 2, 2)
            source_e[:, N:, :N] = self.edge_dim - 2
            source_e[:, :N, N:] = self.edge_dim - 2
            source_e_mask = torch.zeros(B, 2*N, 2*N, device=e.device).bool()
            source_e_mask[:, :N, :N] = True
            e = new_e * source_e_mask + source_e * (~source_e_mask)
            eye_mask = torch.eye(2*N, device=e_onehot.device).bool().unsqueeze(0)
        ...
        if source_o is not None:
            o = o.reshape(B, N, -1)  # (B, N, K)
            o = torch.cat([o, source_o], dim=1)
            o_emb = self.objfeat_embed(o)  # (B, N, K, Do)
            o_emb = o_emb.reshape(B*2*N, 4, -1)  # (B*N, K, Do); TODO: make `4` configurable
            o_emb = o_emb + get_1d_sincos_encode(
                torch.arange(o_emb.shape[1], device=o_emb.device),
                o_emb.shape[-1], o_emb.shape[1]
            ).unsqueeze(0)
            ...
            o_emb = torch.mean(o_emb, dim=1).reshape(B, 2*N, -1)  # (B, N, Do); average over objfeat tokens
        ...
        if source_o is None:
            ...
            out_o_tokens = self.out_objfeat_tokens.unsqueeze(0).repeat(B*2*N, 1, 1)  # (B*N, K, attn_dim)
            out_o_tokens = out_o_tokens + get_1d_sincos_encode(
                torch.arange(out_o_tokens.shape[1], device=out_o_tokens.device),
                out_o_tokens.shape[-1], out_o_tokens.shape[1]
            ).unsqueeze(0)
            for block in self.out_objfeat_transformer:
                out_o_tokens = block(out_o_tokens, context=x_emb.reshape(B*2*N, 1, -1))
            out_o_tokens = self.objfeat_out_norm(out_o_tokens)  # (B*N, K, attn_dim)
            out_o = self.objfeat_out(out_o_tokens)  # (B*N, K, num_objfeat_embeds+1)
            out_o = out_o.reshape(B, 2*N, 4, -1)  # (B, N, K, num_objfeat_embeds+1); TODO: make `4` configurable
