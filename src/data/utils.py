...
def parse_threed_front_scenes(
    ...
    if os.path.exists(preprocessed_file):
        ...
        scenes = pickle.load(open(preprocessed_file, "rb"))
    else:
        ...
        scenes = sum(scenes, [])
        pickle.dump(scenes, open(preprocessed_file, "wb"))
...
def parse_threed_future_models(
    ...
    preprocessed_file = os.path.join(
        os.path.dirname(dataset_directory), "threed_future_model.pkl"
    )
    if os.path.exists(preprocessed_file):
        print(f"Load pickled 3D-FUTURE objects from {preprocessed_file}")
        furnitures = pickle.load(open(preprocessed_file, "rb"))
    else:
        # Parse the model info
        mf = ModelInfo.from_file(path_to_model_info)
        model_info = mf.model_info
        ...
        pickle.dump(furnitures, open(preprocessed_file, "wb"))
