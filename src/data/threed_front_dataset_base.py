...
class JIDEncoder(DataEncoder):
    ...
    def __getitem__(self, idx):
        # Get the scene
        boxes = self._get_boxes(idx)
        jid = [bs.model_jid for bs in boxes]
        jid = np.array(jid)
        return jid
...
class RoomUidEncoder(DataEncoder):
    @property
    def property_type(self):
        return "room_uid"

    def __getitem__(self, idx):
        uid = self._dataset[idx].uid
        return uid
...
class RoomRelationEncoder(DataEncoder):
    @property
    def property_type(self):
        return "relation_path"

    def __getitem__(self, idx):
        return self._dataset[idx].relation_path
    
    @property
    def bbox_dims(self):
        return 1

class OpenShapeIndexEncoder(DataEncoder):
    @property
    def property_type(self):
        return "objfeat_vq_indices"
    ...
    def __getitem__(self, idx):
        ...
        # Get the rotation matrix for the current scene
        L = len(boxes)  # sequence length
        C = len(boxes[0].openshape_vitg14_index())
        latents = np.zeros((L, C), dtype=np.float32)
        for i, bs in enumerate(boxes):
            latents[i, :] = bs.openshape_vitg14_index()
        return latents
...
class OpenShapeReconEncoder(DataEncoder):
    @property
    def property_type(self):
        return "objfeat_vq_recon"
    ...
    def __getitem__(self, idx):
        ...
        # Get the rotation matrix for the current scene
        L = len(boxes)  # sequence length
        C = len(boxes[0].openshape_vitg14_recon())
        latents = np.zeros((L, C), dtype=np.float32)
        for i, bs in enumerate(boxes):
            latents[i, :] = bs.openshape_vitg14_recon()
        return latents
...
class OpenShapeOrigEncoder(DataEncoder):
    @property
    def property_type(self):
        return "objfeat_vitg14_features"
    ...
    def __getitem__(self, idx):
        ...
        # Get the rotation matrix for the current scene
        L = len(boxes)  # sequence length
        C = len(boxes[0].openshape_vitg14_features)
        latents = np.zeros((L, C), dtype=np.float32)
        for i, bs in enumerate(boxes):
            latents[i, :] = bs.openshape_vitg14_features
        return latents
