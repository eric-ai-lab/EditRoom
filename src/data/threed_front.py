...
class ThreedFront(BaseDataset):
    ...
    """
    def __init__(self, scenes, bounds=None, path_to_train_stats=None):
        ...
        if path_to_train_stats:
            self._parse_train_stats(path_to_train_stats)
    ...
    def _compute_bounds(self):
        ...
        for s in tqdm(self.scenes, desc="Computing bounds"):
            for f in s.bboxes:
                if np.any(f.size > 5):
    ...
    def max_length(self):
        if self._max_length is None:
            if isinstance(self.scenes[0], Room):
                _room_types = set([str(s.scene_type) for s in self.scenes])
            else:
                _room_types = [self.scenes[0]]
            for _room_type in _room_types:
                if 'bedroom' in _room_type:
                    self._max_length = 12
                elif 'livingroom' in _room_type:
                    self._max_length = 21
                elif 'diningroom' in _room_type:
                    self._max_length = 21
                elif 'library' in _room_type:
                    self._max_length = 11
    ...
    def from_dataset_directory(cls, dataset_directory, path_to_model_info,
                               ...
                               path_to_bounds=None, path_to_train_stats=None, filter_fn=lambda s: s):
        ...
        return cls([s for s in map(filter_fn, scenes) if s], bounds, path_to_train_stats)

    def _parse_train_stats(self, path_to_train_stats):
        with open(path_to_train_stats, "r") as f:
    ...
    def save_to_folder(self, folder_path, render = False):
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        uid_mapping = {}
        for scene in tqdm(self.scenes, desc="Saving scenes"):
            if render:
                render_path = os.path.join(folder_path, f"{scene.uid}.png")
                scene.direct_render(render_path, orthogonal_view=True)
            scene_save_path = os.path.join(folder_path, f"{scene.uid}.pkl")
            with open(scene_save_path, "wb") as f:
                pickle.dump(scene, f, protocol=pickle.HIGHEST_PROTOCOL)
            if hasattr(scene, "original_id"):
                uid_mapping[scene.uid] = scene.original_id
        with open(os.path.join(folder_path, "uid_mapping.json"), "w") as f:
